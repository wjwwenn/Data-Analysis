...
CitiesCode = {
                ...
                '台湾省':710000,'香港特别行政区':810000,'澳门特别行政区':820000, '全国':0
...
def migration_all_date(areaname,classname,no,direction):
    if no == -1:
        ...
        for date3 in range (20200301, 20200332):
            datelist.append(date3)
        for date4 in range (20200401, 20200431):
            datelist.append(date4)
        for date5 in range (20200501, 20200532):
            datelist.append(date5)
        for date6 in range (20200601, 20200631):
            datelist.append(date6)
        for date7 in range (20200701, 20200732):
            datelist.append(date7)
        for date8 in range (20200801, 20200832):
            datelist.append(date8)
        for date9 in range (20200901, 20200931):
            datelist.append(date9)
        for date in datelist:
            datename = date
            url=f'https://huiyan.baidu.com/migration/cityrank.jsonp?dt={classname}&id={no}&type=move_{direction}&date={date}'
            print(url)
            ...
            if data_dict['errmsg']=='SUCCESS':
                ...
                counter_data += 1
            workbook.save(f"{areaname}-{nameofdire}.xls")
...
if __name__== "__main__":
    circu_exe_direction('全国', 'country', -1)      
    print('done')
